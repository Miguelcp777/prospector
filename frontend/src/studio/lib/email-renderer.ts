...
function renderBlock(
  ...
  switch (block.type) {
    ...
    case "hero": {
      ...
      // El ajuste de la capa de imagen del hero de composición libre. Es
      // otra cosa que `imageFit` de arriba, que es la del hero clásico:
      // aquella ajusta un fondo, y esta una capa colocada a mano.
      const heroLayerFit = ["cover", "contain", "fill"].includes(
      ...
      const freeHeroContent = freeMode
        ? `<div class="hero-free-canvas" style="position:relative;width:100%;height:${height}px;overflow:${props.heroOverflow === "visible" ? "visible" : "hidden"};font-family:${family};text-transform:${transform};">${
            hasImage
              ? `<img data-hero-layer="heroImage" class="hero-free-layer hero-image-layer" src="${image}" alt="${interpolate(props.imageAlt, data)}" style="${layerStyle("heroImage", { x: 72, y: 50, width: 48, z: 2 })}height:${layerNumber("heroImageHeight", 10, 150, 62) * verticalRatio}%;object-fit:${heroLayerFit};object-position:${imagePosition};border:0;border-radius:${Math.min(20, blockRadius)}px;">`
